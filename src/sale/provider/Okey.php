...
class Okey extends Provider {
    ...
    public function getAllSaleItem() {
        $urlBase = 'https://www.okeydostavka.ru/webapp/wcs/stores/servlet/ProductListingView?searchType='
                . '1000&filterTerm=&langId=-20&advancedSearch=&sType=SimpleSearch&gridPosition=&metaData=&manufacturer='
                . '&custom_view=true&ajaxStoreImageDir=%2Fwcsstore%2FOKMarketSAS%2F&resultCatEntryType=&'
                . 'catalogId=12051&searchTerm=&resultsPerPage=72&emsName=&facet=&'
                . 'categoryId=38057&storeId=10151&disableProductCompare=true&'
                . 'ddkey=ProductListingView_6_-1011_3074457345618259713&filterFacet=&beginIndex=';
        ...
        for ($i = 0; $i < 5; $i ++) {
            ...
            $html = file_get_contents($urlBase . ($i * 72), false, $context);
            if (strpos($html, 'product_price') !== false) {
                $content = $this->removeTrash( $html );
                ...
                foreach ($ns as $node) {
                    ...
                    $url = 'https://www.okeydostavka.ru/' . ltrim($a->getAttribute('href'), '/');
                    $hash = md5($url);
    ...
    public function getItemDescription($item) {
        $html = file_get_contents($item->getLink(), false, $this->getContext());

        if (strpos($html, 'tab-container-bordered') !== false) {  //var_dump($html); 
            $p = new \sale\html\DOMParser( $this->removeTrash( $html) );
            $n = $p->parse();
            $ns = \sale\html\Find::find($n, '.tab-container-bordered');
     ...
     * @return type
     */
    private function removeTrash($html) {
